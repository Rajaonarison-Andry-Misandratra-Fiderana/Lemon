...
> **Version source of truth**: the `Version:` line above is the canonical
> project version. `meson.build` parses it via `run_command('sed', ...)` so
> `meson.project_version()`, `-DVERSION="..."` and the binary's `-v` output all
> stay in sync. Bump here, not in `meson.build`.
...
- **Alt+Tab cycler resizes real windows**: `src/dispatch/cycler.h`'s `window_cycler_build` snapshots each eligible client's geometry/state, sets `c->geom` to its grid cell and calls `resize(c, …, 0)` so the existing spring animation moves the live window into place. `ov_anim=true` while inside the cycler so the smoother `animation_spring_overview_{mass,tension,friction}` pair drives it. The overlay on `LyrFadeOut` is *only* a shared 4-strip white outline frame + numbered badges (1..9) — no thumbnail compositing. The dim backdrop lives on `LyrBottom` so real windows draw on top of it. `window_cycler_destroy_with_pick(picked)` restores every backed-up geom/state and triggers `arrange(selmon, false, false)` so the new client order (after any drag-swap) tiles correctly; the wrapper `window_cycler_destroy()` passes `NULL` for non-commit paths. Don't reintroduce snapshot-based thumbnails: real windows give pixel-perfect previews with zero scaling cost.
- **Alt+Tab drag swaps tile order**: pressing BTN_LEFT on a cell + dragging past the 8 px threshold hoists the dragged client to `LyrTop`, follows the cursor via `resize()`, and on release swaps two cells. The swap also pulls the corresponding `Client` entries inside the global `clients` wl_list, so the post-restore `arrange()` lays out tiles in the new order. Pure click (no drag) commits the picked cell. Drag *pursuit* is forced 1:1 with the cursor: after `resize()`, `drag_motion` snaps `c->animation.{current,vis,vel}` onto the live box and sets `running=false`, so the regular soft overview spring does not lag the window behind the pointer.
- **Alt+Tab grid navigation**: `window_cycler_step_grid` walks the grid by `±1` column / row for the four arrow keys; `window_cycler_step` wraps for Tab / Shift+Tab. Both clear `hover = -1` so the white frame switches back from cursor-follow to keyboard-follow on the next press. `window_cycler_hover_at` syncs `index = hit` whenever the mouse hovers a cell, so the keyboard cursor resumes from where the pointer left off. Mouse and keyboard drive one logical selection — neither hijacks the other.
- **Alt+Tab focus frame is a single white outline**: `window_cycler.active_frame[4]` is the only frame ever drawn (hover-or-keyboard-active cell). Filled rects covering whole cells would hide the live windows; the 4 thin strips draw the outline and leave the pixels visible. The spring infrastructure (`frame_pos/vel/target` + `cycler_overlay_tick`) is in place but dormant — `refresh_highlight` snaps the strips to the new cell instantly. Re-enable the spring path by setting `frame_running = true` if you want a smooth transition; the per-frame tick is already hooked into `do_rendermon`.
- **Alt+Tab digit + tag move**: digit detection uses the raw X keycode (`10..18` → digits 1..9) so layouts whose `Shift+1` keysym is `!` (US QWERTY) still match. `Alt+digit` jumps to that cell and commits; `Alt+Shift+digit` moves the *active target* (`hover` if set, else `index`) to workspace N via `window_cycler_move_to_tag`. The mover reverts state in place (no animation flicker), retags, hides via `wlr_scene_node_set_enabled(false)`, compacts the cycler arrays, re-renders the badges with their new numbers and re-applies the (now larger) cells. If the destination workspace is empty, its layout is flipped to `VERTICAL_SCROLLER` so the lone newcomer renders maximized; if it already has residents, the destination layout is left alone — the existing layout tiles the newcomer with them on next `view()`.
- **Alt+Tab includes minimized windows**: `cycler_unminimize_for_show` temporarily restores every minimized client on the active monitor — clears `isminimized`, repaints the tagset to `mini_restore_tag`, and *re-enables* `c->scene` / `c->scene_surface` (set_minimized goes through arrange → set_arrange_hidden which had disabled them). Without the explicit `wlr_scene_node_set_enabled(true)` the cell renders blank and the user has to click twice. `window_cycler_destroy_with_pick` re-minimizes every `ov_was_minimized` client except the one the user picked / moved; `commit` also `view()`s the picked client's `mini_restore_tag` when it differs from the current tagset, so the focused window actually shows up on screen.
- **Alt+Tab sticky mode + spawn passthrough**: while the cycler is active, `keybinding()` (`src/lemon.c`) compares each matched bind's `func` pointer against an allow-list: `spawn` / `spawn_shell` / `spawn_on_empty` and `window_cycler_next/prev` are the only funcs allowed to fire — everything else (`togglemaximizescreen`, `togglefloating`, `togglefullscreen`, `setlayout`, `view`, `tag`, `focusdir`, `exchange_client`, …) is swallowed silently right there. Mirrors overview's per-function `isoverview` early-returns, centralised so adding a new layout bind does not require remembering to gate it. When a spawn bind fires, `window_cycler.sticky` is set so modifier release no longer auto-commits — the cycler stays up until the new client maps (caught by `xdg.h` `mapnotify` → `window_cycler_add_client`) and the user picks it / dismisses with Escape / clicks a cell. Trackpad gestures (`ongesture`) and axis bindings (`axisnotify`) are gated in the same spirit. Key-repeat is also cancelled for non-nav keys so a held bind cannot fire through `keyrepeat()` (which bypasses the dispatch loop).
- **Alt+Tab post-exit collapse**: when the cycler closes with exactly one visible client left on the current workspace (typical after move-to-tag), the workspace layout is flipped to `VERTICAL_SCROLLER` so the survivor renders maximized. Logic in `cycler_count_clients_on_tag` + `cycler_set_tag_layout`.
- **`mod=<modifier>` config alias**: `~/.config/lemon/lemon.conf` can define a `mod=alt|super|ctrl|...` line (default `alt`). Subsequent `bind=MOD,...` / `mousebind=MOD,...` rows expand `MOD` to that mask. The token is parsed inside `parse_mod` (`src/config/parse_config.h`) by checking `config.mod_alias` — added to the existing token cascade. Default `WLR_MODIFIER_ALT`; switch the whole binding table to Super by flipping a single config line.
